			...
			<div class="space-y-4 text-sm text-text-muted">
				<div>
					...
					<code class="block bg-bg-secondary rounded p-3 mt-2 font-mono">
						outlet update --license-key=YOUR_LICENSE_KEY
					</code>
					<p class="mt-2 text-xs">Or set the OUTLET_LICENSE_KEY environment variable.</p>
				</div>
				<div>
					<h3 class="font-medium text-text mb-2">Manual Update</h3>
					<ol class="list-decimal list-inside space-y-1">
						<li>Download the latest release for your platform</li>
						<li>Stop the current Outlet.sh instance</li>
						<li>Replace the binary with the new version</li>
						<li>Start Outlet.sh again</li>
					</ol>
				</div>
			</div>
		</Card>
