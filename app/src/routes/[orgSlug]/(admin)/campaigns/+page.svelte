			...
			{#each tabs as tab}
				...
				>
					...
					{#if statusCounts()[tab.id]}
						<span class="campaigns-tab-count">{statusCounts()[tab.id]}</span>
					{/if}
				</button>
			{/each}
		</div>
...
<style>
	...
	@layer components.campaigns {
		...
		.campaigns-tab.active .campaigns-tab-count {
